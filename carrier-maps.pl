...
print timestamp()."\n$count carriers plotted.\n";

foreach my $image ($rmap,$gmap) {
	
	$image->Draw(primitive=>'rectangle',fill=>'none',stroke=>$graphcolor,strokewidth=>2,points=>"$x1,$y1 $x2,$y2");

	#show_result($image->Annotate(pointsize=>$pointsize,fill=>'white',text=>"$title - ".epoch2date(time)." - $count carriers shown.", gravity=>'northwest', x=>$pointsize, y=>$pointsize));
	...
	annotate_border($image,$pointsize,'white',10,"$title - ".epoch2date(time)." - $count carriers shown.",'northwest',$pointsize,$pointsize);
...
system('/usr/bin/mkdir','-p',"$scriptpath/fleetcarrier-history") if (!-d "$scriptpath/fleetcarrier-history");

my $fn = sprintf("%s/fleetcarrier-history/$fileout-$date.bmp",$scriptpath);
...
my $fn = sprintf("%s/fleetcarrier-history/$fileout-regions-$date.bmp",$scriptpath);
print timestamp()."Writing to: $fn\n";
...
sub get_data {
#	my @rows = db_mysql('elite',"select systemId64,systemName,coord_x,coord_y,coord_z from carriers where (callsign in ('W21-G9Z','W48-Q1Z') or ".
#			"lastEvent>date_sub(NOW(), interval 60 day) or lastMoved>date_sub(NOW(), interval 60 day)) and ((systemName is not null and systemName!='') or systemId64>0)");
	...
	my @rows = db_mysql('elite',"select systemId64,systemName,coord_x,coord_y,coord_z from carriers where ((systemName is not null and systemName!='') or systemId64>0)");
