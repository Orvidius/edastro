...
my $remote_server	= 'www@services:/www/edastro.com/mapcharts/codex/';
...
map_set('surface', 'Crystalline Shards', 'Anemones', 'Bark Mounds','Sinuous Tubers','Amphora Plants','Brain Trees');
#map_set('odyssey', 'Cactoids','Fungoids','Tussocks','Conchas','Strata','Shrubs','Tubus');
...
sub map_set {
	...
	foreach my $i (@list) {
		...
		$color = 'rgb(160,160,160)' if ($n == 11);
		$color = 'rgb(255,255,255)' if ($n == 12);

		$mapkey{$type}{$i} = $color;
	}
}

open SKIP, ">codex-skipped.csv";
print SKIP "Ref.System,Type,Reason\r\n";

open DEBUG, ">codex-debug.txt" if ($debug);
...
my $plusrad=0;
		foreach my $type (sort { int(keys(%{$data{$b}})) <=> int(keys(%{$data{$a}})) } keys %{$map{$maptype}}) {
			foreach my $refsys (keys %{$data{$type}}) {
...
#	}
	...
	foreach my $key (sort keys %{$mapkey{$maptype}}) {
		...
		my $counts = $mapcounts{$maptype}{$key};
		$counts = 0 if (!$counts);
...
my_system("$scp codex-skipped.csv $remote_server/") if (!$debug && $allow_scp);
...
sub get_database {
	...
	foreach my $r (@rows) {
		my $string = $$r{name};
		my @rows2 = db_mysql('elite',"select name from codexname_local where codexnameID=? order by preferred desc",[($$r{id})]);
		if (@rows2) {
			$string = ${$rows2[0]}{name};
		}

		$codexname{$$r{id}} = $string;
		$codextype{$$r{id}} = $$r{name};
	}


	my $ref = rows_mysql('elite',"select systemId64,nameID,name as systemName from codex,systems where systemId64=id64 and codex.deletionState=0 and systems.deletionState=0");

	while (@$ref) {
		my $r = shift @$ref;

		add_codex_entry($$r{systemName},$$r{systemId64},$codextype{$$r{nameID}},$codexname{$$r{nameID}});
	}
}
...
sub do_csv {
	...
	print "\nREAD: $csv_file ($refmatch = $col{ref}, $namematch = $col{name})\n";
	$dot_count = 0;
	...
	while (my $line = <CSV>) {
		...
		if (!defined($col{ref}) || !$ref || !defined($col{name})) {
			print "UNREADABLE: $line\n";
		} else {
			add_codex_entry($ref,$v[$col{id64}],$name,$local);
		}
...
sub add_codex_entry {
	my ($ref,$id64,$name,$local) = @_;

	my $type = '';
#print "Trying: $name ($local)\n" if ($name =~ /Anomal/i || $local =~ /Anomal/i);
...
#print "TUBER: $name {$local} = $t\n" if ($type =~ /tuber/i);
	...
	if (!$type) {
		...
		print DEBUG "NO TYPE: $ref, $id64, $name, $local, $type\n" if ($debug);
		...
		print DEBUG "ADDED: $ref, $id64, $name, $local, $type\n" if ($debug);
	}

	$dot_count++;
	print '.' if ($dot_count % 1000 == 0);
	print "\n" if ($dot_count % 100000 == 0);
}
