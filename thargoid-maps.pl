...
my $bubble_radius	= 600;
my $bubble_diameter	= $bubble_radius*2;
my $bubble_rings	= 6;
my $bubble_ring_ly	= 100;
...
my $title		= "Thargoid Controlled Systems";
...
print "\n".int(keys(%data))." total systems loaded.\nCreating Canvas\n";
...
foreach my $image ($regionmap,$galaxymap) {
	...
	for (my $i=1; $i<=$bubble_rings; $i++) {
		my $rad = floor(($i*$bubble_ring_ly)*($bubble_size_x/$bubble_diameter));
	...
	annotate_border($image,30, $graphcolor, 8, 'Sol', undef, $cx+8,$cy-10);

	for (my $i=1; $i<=$bubble_rings; $i++) {
		my $rad = floor(($i*$bubble_ring_ly)*($bubble_size_x/$bubble_diameter));

		#$image->Annotate(pointsize=>30,fill=>$graphcolor,text=>($i*$bubble_ring_ly).' ly', x=>$cx+8, y=>$cy-$rad-10);
		#$image->Annotate(pointsize=>30,fill=>$graphcolor,text=>($i*$bubble_ring_ly).' ly', x=>$cx+8, y=>$cy+$rad+35);
		#$image->Annotate(pointsize=>30,fill=>$graphcolor,text=>($i*$bubble_ring_ly).' ly', x=>$cx+$rad+8, y=>$cy-10);
		#$image->Annotate(pointsize=>30,fill=>$graphcolor,text=>($i*$bubble_ring_ly).' ly', x=>$cx-$rad+8, y=>$cy-10);

		annotate_border($image,30, $graphcolor, 8, ($i*$bubble_ring_ly).' ly', undef, $cx+8,$cy-$rad-10);
		annotate_border($image,30, $graphcolor, 8, ($i*$bubble_ring_ly).' ly', undef, $cx+8,$cy+$rad+32);
		annotate_border($image,30, $graphcolor, 8, ($i*$bubble_ring_ly).' ly', undef, $cx+$rad+8,$cy-10);
		annotate_border($image,30, $graphcolor, 8, ($i*$bubble_ring_ly).' ly', undef, $cx-$rad+8,$cy-10);
	}
...
print "\n$count thargoid systems plotted.\n";

foreach my $image ($rmap,$gmap) {
	
	$image->Draw(primitive=>'rectangle',fill=>'none',stroke=>$graphcolor,strokewidth=>2,points=>"$x1,$y1 $x2,$y2");

	#show_result($image->Annotate(pointsize=>$pointsize,fill=>'white',text=>"$title - ".epoch2date(time)." - $count thargoid systems shown.", gravity=>'northwest', x=>$pointsize, y=>$pointsize));
	...
	annotate_border($image,$pointsize,'white',10,"$title - ".epoch2date(time)." - $count thargoid systems shown.",'northwest',$pointsize,$pointsize);
...
system('/usr/bin/mkdir','-p',"$scriptpath/thargoid-history") if (!-d "$scriptpath/thargoid-history");

my $fn = sprintf("%s/thargoid-history/$fileout-$date.bmp",$scriptpath);
print "Writing to: $fn\n";
...
my $fn = sprintf("%s/thargoid-history/$fileout-regions-$date.bmp",$scriptpath);
print "Writing to: $fn\n";
...
sub get_data {
	...
	my @rows = db_mysql('elite',"select id64,name,coord_x,coord_y,coord_z from systems where SystemAllegiance=36 and deletionState=0");

	foreach my $r (@rows) {
		$$r{name} = $$r{id64} if ($$r{id64} && !$$r{name});

		$coords{$$r{name}}{x} = $$r{coord_x} if ($$r{name} && defined($$r{coord_x}));
		$coords{$$r{name}}{y} = $$r{coord_y} if ($$r{name} && defined($$r{coord_y}));
		$coords{$$r{name}}{z} = $$r{coord_z} if ($$r{name} && defined($$r{coord_z}));
	
		$id64{$$r{name}} = $$r{id64} if ($$r{name} && $$r{id64});
		$data{$$r{name}}++;
	}
}
