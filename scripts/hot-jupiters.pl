...
my @rows = db_mysql('elite',"select * from planets where subType like '\%giant\%' and orbitalPeriod<=10 and orbitalPeriod>0 and orbitalPeriod is not null ".
			" and (name like '\% 1' or name like '\% 2') and deletionState=0 order by orbitalPeriod desc");
