...
my $search_type = 'Earth-like world';

$search_type = $ARGV[0] if (@ARGV);

my @rows = db_mysql('elite',"select distinct systemId64,count(*) from planets where subType=? and deletionState=0 and ".
		"argOfPeriapsis is not null and orbitalPeriod is not null group by systemId64 having count(*)>1 $limit",[($search_type)]);
while (@rows) {
	my $r = shift @rows;
	push @systems, $$r{systemId64};
}
...
print "System Body Candidates,Distance to Sol,System,Mass Code,System Stars,Primary Star Type,Parent Star,Parent Star Type,".
	"Parent Body,Parent Body Type,Companion Name,Body Name,Type,Landable,Orbit Type,Rings,Arrival Distance,".
...
while (@systems) {
	...
	my @rows =  db_mysql('elite',"select * from planets where systemId64 in ($list) and subType=? and deletionState=0 and ".
					"argOfPeriapsis is not null and orbitalPeriod is not null and semiMajorAxis is not null",[($search_type)]);
...
#						);
			...
			if ($$r{name} ne $system_name) {
				...
				if ($system_name) {
					my @primary = db_mysql('elite',"select name,subType from stars where systemId64=? and name in (?,?) and deletionState=0",[($$r{systemId64},$system_name,"$system_name A")]);
					if (@primary) {
			...
			$out{$p}{$s} = "$solDistance,$system_name,$masscode,$system_type,$primary_type,$star_name,$star_type,$parent_name,$parent_type,$partner,$$r{name},".
				...
				"$$r{distanceToArrivalLS},$$r{terraformingState},$$r{radius},$$r{earthMasses},".
...
warn int(keys %sysfound)." systems listed\n";
