...
$where .= " and planets.deletionState=0" if ($where);
$where =~ s/^\s*and\s+//;
...
my $cols = columns_mysql('elite',"select planetID from planets,systems where $where and systemId64=id64 and systems.deletionState=0");
...
foreach my $r (@colrows) {
	if ($$r{Type} =~ /^(float|double)/) {
		$columns{$$r{Field}} = "cast(planets.$$r{Field} as decimal(65,10)) $$r{Field}";
	} else {
		$columns{$$r{Field}} = $$r{Field};
	}
}

my @colrows = db_mysql('elite',"describe systems");
foreach my $r (@colrows) {
	next if ($columns{$$r{Field}});	# Don't use something if planets has same name

	if ($$r{Type} =~ /^(float|double)/) {
		$columns{$$r{Field}} = "cast(systems.$$r{Field} as decimal(65,10)) $$r{Field}";
	} else {
		$columns{$$r{Field}} = $$r{Field};
	}
}
...
while (@{$$cols{planetID}}) {
	...
	if (!exists($data{$planetID})) {
		...
		my $bodies = rows_mysql('elite',"select ".join(',',values %columns)." from systems,planets where planetID in ($wherelist) and systemId64=id64");
	...
	$out = "$system_name,$masscode,$system_type,$primary_type,$primary_class,$primary_lum,$primary_age,$star_name,$star_type,$star_class,$star_lum,$star_age,".
		"$parent_name,$parent_type,$parent_landable,$parent_terraformable,".
		"$$r{name},$isLandable,$orbit_type,"; 
	$out .= "$$r{subType}," if (!$skip_type);
	$out .= "$ringnum,\"$ringtypes\",".
		...
		"$$r{orbitalInclinationDec},$$r{argOfPeriapsisDec},$$r{meanAnomaly},$$r{ascendingNodeDec},$$r2{coord_x},$$r2{coord_y},$$r2{coord_z},$$r{updateTime}\r\n";

	$out = "$$r{name},$$r{subType},$$r{updateTime},$$r{date_added}\r\n" if ($simple_list && !$skip_type);
	$out = "$$r{name},$$r{updateTime},$$r{date_added}\r\n" if ($simple_list && $skip_type);
