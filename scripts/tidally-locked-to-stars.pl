...
my $cols = columns_mysql('elite',"select planetID,p.edsmID,p.name,p.subType,p.rotationalPeriod,p.orbitalPeriod,s.subType startype,s.name starname ".
				"from planets p,stars s where p.rotationalPeriodTidallyLocked=1 and ".
				"p.subType in ('Earth-like world','Ammonia world','Water world') and ".
				"p.name rlike ' [0-9]\$' and s.name=regexp_replace(p.name, ' [0-9]\$', '') and ".
				"p.rotationalPeriod>0 and p.orbitalPeriod>0 and p.rotationalPeriod is not null and p.orbitalPeriod is not null and ".
				"abs(p.rotationalPeriod-p.orbitalPeriod)<0.0001 and p.deletionState=0 and s.deletionState=0 ".
				"order by p.name");

print make_csv('EDSM ID','Name','Type','Rotational Period','Orbital Period','Difference','Parent Name','Parent Type')."\r\n";
...
for(my $i=0; $i<@{$$cols{planetID}}; $i++) {
	...
	print make_csv(${$$cols{edsmID}}[$i],${$$cols{name}}[$i],${$$cols{subType}}[$i],${$$cols{rotationalPeriod}}[$i],${$$cols{orbitalPeriod}}[$i],
			$diff,${$$cols{starname}}[$i],${$$cols{startype}}[$i])."\r\n";
}
