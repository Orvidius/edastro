...
my $title		= "Sample maps";
my $author		= "Map by CMDR Orvidius (edastro.com)";
...
my $f = sprintf("%s/$outname.bmp",$filepath);
print "Writing to: $f\n";
...
my $f = sprintf("%s/$outname-regions.bmp",$filepath);
print "Writing to: $f\n";
...
my_system("$scp $skip_file $remote_server/") if ($skip_file && !$debug && $allow_scp);
...
sub do_sql {
	...
	die "Need SQL pattern\n" if (!$sql_select);
	my $rows = rows_mysql('elite',$sql_select);

	while (@$rows) {
		my $r = shift @$rows;

		$systems{$$r{id64}} += $$r{num};
