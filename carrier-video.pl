...
my $path	= '/home/bones/elite/fleetcarrier-history';
...
my $output	= '/home/bones/elite/carrier-video.mp4';
...
foreach my $regioncode ('','-regions') {
	...
	while (my $fn = readdir DIR) {
		if ($fn =~ /^carriers$regioncode-\d+.png$/) {
			$files{"$path/$fn"} = 1;
		}
	}
	closedir DIR;
	...
	while (my $fn = readdir DIR) {
		if ($fn =~ /^carriermap$regioncode-(\d+).png$/) {
	...
	foreach my $fn (sort keys %files) {
		...
		my $lnf = "$seq_path/carriermap$regioncode-$n.png";
		$links{$lnf} = $fn;
		print "ADDING: $fn -> $lnf\n";
		system($ln,'-s',$fn,$lnf);
	}
	...
	my $syscall = "$ffmpeg -y -framerate $fps -i $seq_path/carriermap$regioncode-%06d.$format -c:v libx264 -profile:v high -crf 20 -pix_fmt yuv420p -vf scale=2560x1280 $fn";
	print "$syscall\n";
